    ...
    <div className="group relative overflow-hidden rounded-2xl border border-[#c9a85c]/25 glass shadow-deep">
        ...
        {!playing ? (
          <>
            <img
              ...
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0b4065] via-[#0b4065]/30 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-r from-[#0b4065]/60 via-transparent to-[#0b4065]/60" />
            <button
              type="button"
              onClick={handlePlay}
            ...
            >
              <span className="relative flex h-20 w-20 items-center justify-center rounded-full border border-[#c9a85c]/60 bg-[#0b4065]/40 backdrop-blur-md transition-all duration-500 group-hover/btn:scale-110 group-hover/btn:border-[#d4af37]">
            ...
            <div className="absolute left-5 top-5 flex items-center gap-2">
              <span className="rounded-full border border-[#c9a85c]/40 bg-[#0b4065]/70 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.25em] text-[#d4af37]">
                {officialBadge}
              </span>
            </div>
          </>
      ...
      {(description || title) && (
        <div className="flex flex-col gap-2 p-6">
          ...
          {description && (
            <p className="text-sm leading-relaxed text-[#8fb4d6]">{description}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      {expanded && (
        ...
        >
          <div className="absolute inset-0 bg-[#0b4065]/95 backdrop-blur-xl" />
          <button
            className="absolute right-6 top-6 z-10 rounded-full border border-[#c9a85c]/40 bg-[#0b4065]/70 p-3 text-[#f4e9c9] transition hover:bg-[#c9a85c]/15"
            onClick={() => setExpanded(false)}
